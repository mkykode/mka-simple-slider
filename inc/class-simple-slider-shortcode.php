...
class SimpleSliderShortcode {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'flickity-css-custom' );
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				if ( ! empty( $large_image_url[0] ) ) {
					$bg = $large_image_url[0];

				}
				echo '<div class="carousel-cell">';
				echo '<div class="carousel-wrap" style="background-image:url(' . esc_url( $bg ) . ')">';
	...
	public static function run( SimpleSliderOptions $options ) {
		...
		add_shortcode( 'mka-simple-slider', array( $shortcode, 'display_shortcode' ) );
